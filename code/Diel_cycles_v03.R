...
# ───────────────────────────────────────────────────────────────────────────────
# long format + diel statistics (mean ± SD per variable, per year)
diel_df <- df %>%
  pivot_longer(
  ...
  drop_na(value, hour, month, year) %>%
  group_by(year, month, hour, variable) %>%
  summarise(
...
p <- ggplot(diel_df, aes(x = hour_num)) +
  ...
  theme_bw() +
  labs(
    title = "Diel Cycles at Tambopata: FCH4, H, LE and NEE",
    x = "Hour of Day",
...
# ───────────────────────────────────────────────────────────────────────────────
# PLOT: FCH4 only, by year WITH SD ribbons
diel_FCH4 <- diel_df %>% 
  filter(variable == "FCH4")

p_FCH4 <- ggplot(diel_FCH4, aes(x = hour_num)) +
  # SD ribbons grouped by year
  geom_ribbon(
  ...
  theme_bw() +
  labs(
    title = "Diel Cycle at Tambopata: FCH4 by Year (Mean ± SD)",
    x = "Hour of Day",
    y = "FCH4 (nmol m\u207b\u00b2 s\u207b\u00b9)",
    colour = "Year",
    fill   = "Year"
...
ggsave(
  filename = paste0(graphs_path, "diel_FCH4_by_year_with_SD.png"),
  plot = p_FCH4,
  width = 12,
  height = 6,
  dpi = 300
...
# PLOT: FCH4 only, by year WITH SD ribbons, Y-axis limited
p_FCH4_limited <- ggplot(diel_FCH4, aes(x = hour_num)) +
  geom_ribbon(
    aes(ymin = lower, ymax = upper, group = year, fill = year),
    alpha = 0.20,
    colour = NA
  ...
  theme_bw() +
  labs(
    title = "Diel Cycle at Tambopata: FCH4 by Year (Mean ± SD), Y-Limited",
    x = "Hour of Day",
    y = "FCH4 (nmol m\u207b\u00b2 s\u207b\u00b9)",
    colour = "Year",
    fill   = "Year"
...
ggsave(
  filename = paste0(graphs_path, "diel_FCH4_by_year_with_SD_ylim_-20_20.png"),
  plot = p_FCH4_limited,
  width = 12,
  height = 6,
  dpi = 300
...
# ───────────────────────────────────────────────────────────────────────────────
# NEW PLOT: FCH4, single panel, lines coloured by month (all years combined)
diel_FCH4_month <- diel_FCH4 %>%
...
p_FCH4_month_lines <- ggplot(diel_FCH4_month,
  ...
  theme_bw() +
  labs(
    title  = "Diel Cycle at Tambopata (upland forest - terra firme): FCH4 by Month (All Years Combined)",
    x      = "Hour of Day",
    y      = "FCH4 (nmol m\u207b\u00b2 s\u207b\u00b9)",
    colour = "Month"
  ) +
